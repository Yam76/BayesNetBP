...
shinyServer(function(input, output, session) {
  ...
  observe({
    ...
    values <- BayesNetBP:::GetValue(v$tree.post, x, message=FALSE)
  ...
  output$plot <- renderCyjShiny({
    ...
    temp_dag <- subGraph(nodeData.2$name, dag)
    # network <- createCytoscapeJsNetwork(nodeData.2, edgeData.2)
    cyjShiny(temp_dag, "dagre")
    # print(nodeData)
  })

  output$plot1 <- renderCyjShiny({
